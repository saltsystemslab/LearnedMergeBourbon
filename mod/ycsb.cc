...
void ycsb_load_run_string( int wl, int kt, int ap, int num_thread,
...
void ycsb_load_run_randint(int wl, int kt, int ap, 
        ...
        std::vector<int> &ops)
{
    ...
    if (ap == UNIFORM) {
        ...
        } else if (kt == RANDINT_KEY && wl == WORKLOAD_F) {
            init_file = "../../RECIPE/index-microbench/workloads/loadf_unif_int.dat";
            txn_file = "../../RECIPE/index-microbench/workloads/txnsf_unif_int.dat";
        } 
    } else {
        if (kt == RANDINT_KEY && wl == WORKLOAD_A) {
            init_file = "../../RECIPE/index-microbench/workloads/loada_unif_int.dat";
    ...
    cout << keys[0] << endl;
    fprintf(stderr, "populated in run %d keys\n", count);
...
std::vector<std::vector<uint64_t>> ycsb_main
(
...
//     }
    ...
    int wl;
    if (strcmp(workload_type, "a") == 0) {
        wl = WORKLOAD_A;
    } else if (strcmp(workload_type, "b") == 0) {
        wl = WORKLOAD_B;
    } else if (strcmp(workload_type, "c") == 0) {
        wl = WORKLOAD_C;
    } else if (strcmp(workload_type, "d") == 0) {
        wl = WORKLOAD_D;
    } else if (strcmp(workload_type, "e") == 0) {
        wl = WORKLOAD_E;
    } else if (strcmp(workload_type, "f") == 0) {
        wl = WORKLOAD_F;
    }
     else {
    ...
    int ap;
    if (strcmp(access_pattern, "uniform") == 0) {
        ap = UNIFORM;}
    else if (strcmp(access_pattern, "zipfian") == 0) {
         ap = ZIPFIAN;
    } else {
   ...
   // int num_thread = atoi(argv[5]);
    ...
    if (kt != STRING_KEY) {
        ...
        //      ycsb_keys.push_back(keys[i]);
        //  }

        // cout << "--ycsb---"  << endl;
        // cout << ycsb_keys[0] << endl;
        // cout << "-----"  << endl;
    } else {
        // std::vector<Key *> init_keys;
        // std::vector<Key *> keys;
