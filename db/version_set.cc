    ...
    if (level == 0) {
      ...
      if (adgMod::MOD == 9) {
        ...
        adgMod::LearnedIndexData* learned_this_level =
            learned_index_data_[level].get();
        if (learned_this_level->Learned(this, adgMod::db->version_count,
                                        level)) {
          adgMod::levelled_counters[13].Increment(level);
          // use level model to get the target file
          learned = true;
          std::pair<uint64_t, uint64_t> bounds =
              learned_this_level->GetPosition(user_key);
          //                        size_t index;
          if (bounds.first <= learned_this_level->MaxPosition()) {
            files = nullptr;
            num_files = 0;
            for (int i = bounds.first;
                 i <= bounds.second && i < files_[level].size(); ++i) {
    ...
    for (uint32_t i = 0; i < num_files; ++i) {
      ...
      if (adgMod::MOD == 0 || adgMod::MOD == 8) {
        ...
        s = vset_->table_cache_->Get(
            options, f->number, f->file_size, ikey, &saver, SaveValue, level, f,
      ...
      adgMod::learn_cb_model->AddLookupData(
          level, saver.state == kFound, file_learned, temp.second - temp.first);
      //        if (model != nullptr) {
      //            model->FillCBAStat(saver.state == kFound, file_learned,
...
int64_t Version::GetLimit(const ReadOptions& options, int &file_count, const Comparator* comparator, const Slice &target_key, const LookupKey& k,
 ...
 // adgMod::Stats* instance = adgMod::Stats::GetInstance();
  ...
  Slice user_key = k.user_key();
  //std::cout<<"ikey size: "<<ikey.size()<<" user key size: "<<user_key.size()<<std::endl;
  const Comparator* ucmp = vset_->icmp_.user_comparator();
  Status s;
 ...
 // stats->seek_file_level = -1;
  ...
  std::vector<FileMetaData*> tmp;
  FileMetaData* tmp2;
    size_t num_files = allFiles.size();
    if (num_files == 0) return 0;

    // Get the list of files to search in this level
    FileMetaData* const* shortlisted_files = &allFiles[0];
    uint64_t position_lower = 0;
    uint64_t position_upper = 0;
    bool learned = false;
    // Step FindFile
   // instance->StartTimer(0);
    if (level == 0) {
      ...
      // So we follow the original LevelDB way in FindFile process
      tmp.reserve(num_files);
      FileMetaData* f = shortlisted_files[0];
      tmp.push_back(f);
      // for (uint32_t i = 0; i < num_files; i++) {
      //   FileMetaData* f = shortlisted_files[i];
      //    tmp.push_back(f);
      //   // if (ucmp->Compare(user_key, f->smallest.user_key()) >= 0 &&
      //   //     ucmp->Compare(user_key, f->largest.user_key()) <= 0) {
      //   //       //std::cout<<"smallest key in shortlisted file"<<f->smallest.user_key().ToString()<<std::endl;
      //   //       //std::cout<<"largest key in shortlisted file"<<f->largest.user_key().ToString()<<std::endl;
      //   //       //std::cout<<"level 0 :file number while adding to shortlistedFiles"<<f->number<<std::endl;
      //   //   tmp.push_back(f);
      //   // }
      // }
    //   if (tmp.empty()) {
      ...
      if (adgMod::MOD == 9) {
        ...
        if (learned_this_level->Learned(this, adgMod::db->version_count,
          ...
          if (bounds.first <= learned_this_level->MaxPosition()) {
            ...
            num_files = 0;
            for (int i = bounds.first;
                 i <= bounds.second && i < allFiles.size(); ++i) {
                  file_count++;
              FileMetaData* file = allFiles[i];
              if (ucmp->Compare(file->smallest.user_key(), user_key) <= 0 &&
                  ucmp->Compare(file->largest.user_key(), user_key) >= 0) {
                    file_count--;
                shortlisted_files = &allFiles[i];
                num_files = 1;
                break;
              }
            }
...
void VersionSet::ApplyToModel(VersionEdit* edit, Version* previous,
                              Version* current) {
...
Status VersionSet::WriteSnapshot(log::Writer* log) {
  ...
  for (int level = 0; level < config::kNumLevels; level++) {
    ...
    for (size_t i = 0; i < files.size(); i++) {
      const FileMetaData* f = files[i];
      edit.AddFile(level, f->number, f->file_size, f->num_keys, f->smallest, f->largest);
    }
  }
...
void Version::PrintAll() const {
  for (int i = 1; i < config::kNumLevels; ++i) {
    for (int j = 0; j < files_[i].size(); ++j) {
      ...
      string small_key = string(file->smallest.user_key().data(),
                                file->smallest.user_key().size());
      string large_key = string(file->largest.user_key().data(),
                                file->largest.user_key().size());
      printf(
          "File %d in level %d:\n"
          "\tNumber: %lu\n"
          "\tSize: %lu\n"
          "\tNumEntries: %lu\n"
          "\tKey Range: %s to %s\n",
          j, i, file->number, file->file_size, 0ul, small_key.c_str(),
          large_key.c_str());
    }
  }
}

bool Version::FillData(const ReadOptions& options, FileMetaData* meta,
...
bool Version::FillLevel(const ReadOptions& options, int level) {
  ...
  for (int j = 0; j < files_[level].size(); ++j) {
    ...
    data->string_keys.emplace_back(file->smallest.user_key().data(),
                                   file->smallest.user_key().size());
    data->string_keys.emplace_back(file->largest.user_key().data(),
                                   file->largest.user_key().size());
  }
...
void Version::WriteLevelModel() {
  // return;
  for (int i = 0; i < config::kNumLevels; ++i) {
    learned_index_data_[i]->WriteModel(vset_->dbname_ + "/" + to_string(i) +
                                       ".model");
    for (FileMetaData* file_meta : files_[i]) {
      adgMod::file_data->GetModel(file_meta->number)
          ->WriteModel(vset_->dbname_ + "/" + to_string(file_meta->number) +
                       ".fmodel");
    }
  }
}
...
void Version::ReadLevelModel() {
  ...
  for (int i = 0; i < config::kNumLevels; ++i) {
    if (adgMod::load_level_model)
      learned_index_data_[i]->ReadModel(vset_->dbname_ + "/" + to_string(i) +
                                        ".model");

    for (FileMetaData* file_meta : files_[i]) {
      if (adgMod::load_file_model) {
        adgMod::file_data->GetModel(file_meta->number)
            ->ReadModel(vset_->dbname_ + "/" + to_string(file_meta->number) +
                        ".fmodel");
      }
  ...
  for (int i = 0; i < config::kNumLevels; ++i) {
    for (FileMetaData* file_meta : files_[i]) {
      adgMod::file_stats.insert(
          {file_meta->number, adgMod::FileStats(i, file_meta->file_size)});
      file_max = file_max > file_meta->number ? file_max : file_meta->number;
    }
  }
  adgMod::file_data->watermark = file_max;
...
void Version::FileLearn() {
  for (int i = 0; i < config::kNumLevels; ++i) {
    for (FileMetaData* file_meta : files_[i]) {
      adgMod::LearnedIndexData::FileLearn(new adgMod::MetaAndSelf{
          this, adgMod::db->version_count, file_meta,
          adgMod::file_data->GetModel(file_meta->number), i});
    }
  }
}
